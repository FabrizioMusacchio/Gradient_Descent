"""
A simple script to demonstrate gradient descent. The script is based on the blog post 
"Visualizing the gradient descent method" (https://scipython.com/blog/visualizing-the-gradient-descent-method/){{site.data.scuts.extlink}} 
from scipython.com. 
...
alpha = 1.15
theta1 = [0.0]
J = [np.array(cost_func(theta1[0])[0])]
for j in range(N-1):
    ...
    J.append(np.array(cost_func(this_theta1)[0]))
...
for j in range(1,N):
    ax[1].annotate('', xy=(theta1[j], J[j]), xytext=(theta1[j-1], J[j-1]),
                   ...
                   va='center', ha='center')
    ax[0].plot(x, hypothesis(x, theta1[j]),  lw=2,
               label=r'$\theta_1 = {:.3f}$'.format(theta1[j]))
    #plt.savefig('animation/gradient_descent_1D_{:03d}.png'.format(j), dpi=120)

# Labels, titles and a legend.
#ax[1].scatter(theta1, J, c=colors, s=40, lw=0)
ax[1].scatter(theta1, J,  s=40, lw=0)
ax[1].set_xlim(-0.2,1)
